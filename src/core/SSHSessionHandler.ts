...
export class SSHSessionHandler extends EventEmitter {
  ...
  private async connectSSH(
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('error', (err) => {
        clearTimeout(timeout);
        this.logger.error(
          `SSH connection error for session ${sessionId}:`,
          err
        );
        reject(err);
      });
   ...
   */
  private async createShellChannel(
    client: SSHClient,
    options: SSHSessionOptions,
    sessionId: string
    ...
    return new Promise((resolve, reject) => {
      ...
      client.shell(ptyOptions, (err, channel) => {
        if (err) {
          this.logger.error(
            `Failed to create shell for session ${sessionId}:`,
            err
          );
          reject(err);
          return;
  ...
  async executeCommand(sessionId: string, command: string): Promise<void> {
    ...
    if (isInteractive) {
      ...
      this.logger.info(
        `Entering interactive mode for session ${sessionId}: ${command}`
      );
    }
  ...
  async sendKey(sessionId: string, key: string): Promise<void> {
    ...
    this.logger.debug(
      `Sending key '${key}' (${this.escapeSequence(sequence)}) to session ${sessionId}`
    );
  ...
  private detectInteractiveMode(session: SSHSession, output: string): void {
    ...
    const hasInteractivePattern = interactivePatterns.some((pattern) =>
      pattern.test(output)
    );
